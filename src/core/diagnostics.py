...
class DiagnosticsManager:
        ...
        try:
            ...
            if "RUNNING" in result.stdout:
                return DiagnosticResult(
                    name="Base Filtering Engine", status="ok", message="Запущен"
                )
            else:
                return DiagnosticResult(
                    name="Base Filtering Engine", status="error", message="Не запущен", fixable=True
                )

        except Exception as e:
            logger.error(f"Ошибка проверки BFE: {e}")
            return DiagnosticResult(
                name="Base Filtering Engine", status="error", message=f"Ошибка проверки: {e}"
            )
    ...
    def check_tcp_timestamps(self) -> DiagnosticResult:
        ...
        try:
            ...
            if "timestamps" in result.stdout.lower() and "enabled" in result.stdout.lower():
                return DiagnosticResult(name="TCP timestamps", status="ok", message="Включены")
            else:
                return DiagnosticResult(
                    name="TCP timestamps",
                    status="warning",
            ...
            return DiagnosticResult(
                name="TCP timestamps", status="error", message=f"Ошибка проверки: {e}"
            )
    ...
    def check_windivert_driver(self) -> DiagnosticResult:
        ...
        if not missing:
            return DiagnosticResult(name="WinDivert драйвер", status="ok", message="Найден")
        else:
            return DiagnosticResult(
    ...
    def check_conflicting_services(self) -> DiagnosticResult:
        ...
        if not conflicts:
            return DiagnosticResult(
                name="Конфликтующие сервисы", status="ok", message="Не обнаружены"
            )
        else:
            return DiagnosticResult(
    ...
    def check_vpn_services(self) -> DiagnosticResult:
        ...
        try:
            ...
            if not vpn_services:
                return DiagnosticResult(name="VPN сервисы", status="ok", message="Не обнаружены")
            else:
                return DiagnosticResult(
                    name="VPN сервисы",
                    status="warning",
            ...
            return DiagnosticResult(
                name="VPN сервисы", status="error", message=f"Ошибка проверки: {e}"
            )
    ...
    def check_killer_network(self) -> DiagnosticResult:
        ...
        try:
            ...
            if "Killer" in result.stdout:
                ...
                return DiagnosticResult(name="Killer Network", status="ok", message="Не обнаружен")

        except Exception as e:
            logger.error(f"Ошибка проверки Killer Network: {e}")
            return DiagnosticResult(
                name="Killer Network", status="error", message=f"Ошибка проверки: {e}"
            )
    ...
    def check_adguard(self) -> DiagnosticResult:
        ...
        try:
            ...
            if "AdguardSvc.exe" in result.stdout:
                return DiagnosticResult(
                    name="Adguard", status="warning", message="Запущен (может мешать Discord)"
                )
            else:
                return DiagnosticResult(name="Adguard", status="ok", message="Не обнаружен")

        except Exception as e:
            logger.error(f"Ошибка проверки Adguard: {e}")
            return DiagnosticResult(name="Adguard", status="error", message=f"Ошибка проверки: {e}")
